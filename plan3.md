...
| --- | --- | --- |
| Which datasets to ingest | `config/cbs.json` → `dataObjectMappings` | JSON (DWA schema) |
...
00:00 UTC      Airflow fires daily_change_detector
               │
               ▼
Load config/cbs.json → iterate enabled dataObjectMappings
...
1. Write `config/cbs.json` with `dataObjectMappings` for `84583NED` and `85523NED` (as shown above).
...
To onboard a non-CBS source (e.g. a future PDOK OData service), create a new file `config/pdok.json` with its own `dataObjectMappings` and point the DAGs at both files. The Python code remains untouched.
