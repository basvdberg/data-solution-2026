...
> **Superseded for KNMI daily data.** The haleconnect INSPIRE WFS used in the original PoC no longer receives new observations (data ends ~2023-09-01). Use [KNMI Data Platform Open Data](../knmi/README.md) and `data-object-mapping/staging/knmi/knmi-daggegevens.json` instead. See [stale WFS note](../../readme.md#stale-knmi-wfs-feed-superseded).
...
- **`probe_change_marker(base_url, type_name, ...)`** — lightweight `GetFeature`
  with a small `count`; returns max `period_end` from returned GML members.
  Used by `poller` for change detection (no full extract).
...
- **`extract_max_period_end(gml_bytes)`** — reads max `gml:endPosition` per
  `PointTimeSeriesObservation` member; poller-only, no TVP flattening.

  Full extract output schema (`parse_timeseries_observations`):
...
python -m extractor.wfs --mapping knmi-daggegevens-temperature --info

# Extract 4 stations, 2 per page, cap at 500 parsed records
python -m extractor.wfs --mapping knmi-daggegevens-temperature --page-size 2 --max-features 4 --limit 500

# Extract with default page size from config
python -m extractor.wfs --mapping knmi-daggegevens-temperature
```

## Configuration

The extractor is driven by a DWA-style JSON mapping stored at
`data-object-mapping/knmi-daggegevens.json`. Key extensions on the mapping:
...
3. Create a mapping JSON in `data-object-mapping/` with `gml_parser` set to
   `<name>`.
